...
如果 AI 把 Excel 的「萬元」看成「元」，直接說：

```text
這個回答錯了
```

系統不會直接相信 AI 的新說法，而是走這條路：

```text
重新核對剛才的原始 Excel 證據
→ 有證據才建立修正
→ 修正只存在本機 KnowledgeBase/50_corrections/
→ 下次相似問題 prepare 自動帶入
→ AI 必須說明有沒有套用
→ finalize 再次檢查後才准保存
```

判斷權力永遠是：`原始 Excel 證據 ＞ 修正紀錄 ＞ AI 自己的猜測`。修正不能代替原始
證據，也不能憑空創造答案。

新 Excel 進來時，舊修正會自動重驗：

- `active`：仍適用，回答前會自動提醒 AI。
- `stale`：表格可能改版，先等待確認。
- `suspended`：發現例外或由你要求暫停，不再自動套用。
- `retired`：規則已永久退役，只保留歷史。

錯誤回答本身仍不會自動保存。只有使用者明確回報「這個回答錯了」，或電腦能驗證的
引用、算術、單位錯誤，才可能建立修正。
